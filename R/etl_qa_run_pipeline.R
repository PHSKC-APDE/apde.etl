...
#'   `chi_standard_varnames`. Default is `FALSE`
#'
...
#' - `chi_standards`: Comparison of CHI (Community Health Indicator) variables values with those expected based on `chi_standard_varnames`
#'
#' @examples
#' \dontrun{
#'
...
process_r_dataframe <- function(config) {
  ...
  if (config$data_source_type == 'r_dataframe') {
    ...
    if(isTRUE(config$data_params$check_chi)){
      byvars <- unique(chi_standard_varnames$varname)
  ...
  if(isTRUE(config$data_params$check_chi)){
    ...
    chi_std <- unique(chi_standard_varnames[, list(varname, group, chi = 1L)])
...
## comp_2_chi_std() ----
#' Compare data to CHI standards in chi_standard_varnames
#'
...
comp_2_chi_std <- function(myCHIcomparison, time_var){
  ...
  if (nrow(only_chi) > 0){
    message("\U0001f626\U0001f47f\U0001F92C\u26A0\ufe0f \n",
            "The following varname and group combinations exist in the chi_standard_varnames \n",
  ...
  if (nrow(only_your_data) > 0){
    message("\U0001f626\U0001f47f\U0001F92C\u26A0\ufe0f \nThe following varname & group combinations in your table are not valid\n",
            "when compared to chi_standard_varnames:\n\n",
            paste(formatted_table2, collapse = "\n"), '\n')
  }

  # Give message of success if there are no problems ----
  if (nrow(only_your_data) == 0 && nrow(only_chi) == 0){
    message("\U0001f973\U0001f389\nAll of the CHI variables found in your dataset are formatted according to the standards in chi_standard_varnames!\n")
  }
