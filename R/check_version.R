#' Check if the installed apde.etl package version is the latest available
#'
#' @description
#' This function checks if the installed version of the apde.etl package is the latest
...
#'
#' @note
#' This function replaces the deprecated `check_version` function from the
#' `apde.etl` package.
#'
...
#'   \item{local_version}{The version of the installed apde.etl package.}
...
#' @examples
#' \dontrun{
#' # Check if apde.etl is up to date
...
check_version <- function(print_message = TRUE) {
  ...
  tryCatch({
    ...
    url <- "https://raw.githubusercontent.com/PHSKC-APDE/apde.etl/main/DESCRIPTION"
    resp <- httr::GET(url)
    ...
    if (httr::status_code(resp) == 200) { # 200 means is okay / was successful
      ...
      if (!is.null(desc_vals) && "Version" %in% colnames(desc_vals)) {
        ...
        # Get local version
        local_version <- utils::packageVersion("apde.etl")
        ...
        if (remote_version > local_version) {
          ...
          update_msg <- ifelse(requireNamespace("devtools", quietly = TRUE),
                               'To update, run: devtools::install_github("PHSKC-APDE/apde.etl", auth_token = NULL)',
                               'To update, install devtools and then run: devtools::install_github("PHSKC-APDE/apde.etl", auth_token = NULL)'
          )
          ...
          if (print_message) {
            message("-----------------------------------------------------")
            message("\u26A0\ufe0f A newer version of apde.etl is available: ", remote_version)
            message("Your version: ", local_version)
            message('\U0001f504 ', update_msg)
            message("-----------------------------------------------------")
          }
        } else {
          result$message <- paste0("Package apde.etl is up to date (version ", local_version, ").")
          if (print_message) {
            message(result$message)
          }
        }
